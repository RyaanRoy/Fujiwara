...
module.exports.run = async (client, message, args) => {
	...
	if (!ban.get(member.id))
		return message.channel.send({embeds:[embed1]}).then(m => m.delete({ timeout: 15000 }));
	...
	const EMDDD = new Discord.MessageEmbed()
		...
		.setColor("GREEN");
	return message.guild.members.unban(member).then(message.channel.send({embeds:[EMDDD]}));
};

module.exports.help = {
	name: "unban",
	description: "This command is used for unbanning someone",
	usage: "f-unban <USER ID>",
